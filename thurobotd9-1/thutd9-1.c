...
void main()
{
   ...
   WHILE (true)
   {
      ...
      lcd_putc ('\f');
      if (start == 0) {trisctp = 1; ctp=1; WHILE (ctp == 1){dcpn (400); }; resetdcp () ; start = 1; delay_ms (500);  };
      WHILE (start == 1) {doctayPES (); lcd_gotoxy (0, 1); lcd_putc (" Ready2..."); if (select == 0) {trisvanp = 0; vanp = 0; trisvant = 0;  vant=0; } };
      lcd_putc ('\f');
      if (start == 0) {trisctt = 1; ctt = 1; WHILE (ctt == 1) {dctt (400); }; resetdct(); start = 1; endatatn = 0; delay_ms (500);  };
      WHILE (start == 1) {doctayPES (); lcd_gotoxy (0, 1); lcd_putc (" Ready3..."); if (select == 0) {trisvanp = 0; vanp = 0; trisvant = 0;  vant=0; } };
      lcd_putc ('\f');
      
      if (start == 0) {endatatn = 0; WHILE (endatatn < 50) {dctn (400); }; resetdct (); start = 1; delay_ms (500);  };
      ...
      if (start == 0) {trisctt = 1; ctt = 1; WHILE (ctt == 1) {dctt (400); }; resetdct(); start = 1; delay_ms (500);  };
