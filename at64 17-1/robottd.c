...
if (kenhb1==0) {ENDATA1++;} else {ENDATA1--;}
}
...
{
// Place your code here
if (kenhb2==0) {ENDATA2++;} else {ENDATA2--;}
}
...
void PIDDC1(volatile long int SETVT1)
     ...
     if (REALVT1 < SETVT1) 
          {
          ERR1=SETVT1-REALVT1; 
          if (ERR1>500) {DC1THUAN(500);} else {DC1THUAN(ERR1+50);}
          }
     if (REALVT1 > SETVT1) 
          {
          ERR1=REALVT1-SETVT1; 
          if (ERR1>500) {DC1NGUOC(500);} else {DC1NGUOC(ERR1+50);}
          }
...
void PIDDC2(volatile long int SETVT2)
     ...
     if (REALVT2 < SETVT2) 
          {
          ERR2=SETVT2-REALVT2; 
          if (ERR2>500) {DC2THUAN(500);} else {DC2THUAN(ERR1+50);}
          }
     if (REALVT2 > SETVT2) 
          {
          ERR2=REALVT2-SETVT2; 
          if (ERR1>500) {DC2NGUOC(500);} else {DC2NGUOC(ERR1+50);}
          }
...
// Clock value: 10.800 kHz   CHU KY XUNG 0.926 MS
...
while (1)
     ...
     while (nut1==1) {DOCPS2(); lcd_putnum(ENDATA1,0,0); lcd_putnum(ENDATA2,8,0); delay_ms(100);}
          ...
          nut1=1;
          BEEP(); 
          while (CTHT2==1) {DC2THUAN(400); lcd_putnum(ENDATA2,0,0); }; 
          RESDC2();
          for (i=0; i<3; i++) {BEEP(); }                                                                              
     ...
     while (nut1==1) {DOCPS2(); lcd_putnum(ENDATA1,0,0); lcd_putnum(ENDATA2,8,0); delay_ms(100);}
          ...
          DOCPS2();
          if (up==0) { SETVT1++; BEEP(); } 
          if (down==0) { SETVT1--; BEEP(); }
          if (left==0) { SETVT2++; BEEP(); }
          if (right==0) { SETVT2--; BEEP(); }
